...
saveext = "~/Desktop/Thorsten/FINAL/Clara_Amnioids_Figuers/"
dir.create(saveext)
...
expe2 <- c("EmDisc_#1","EmDisc_#2","Amnioid_#1","Amnioid_#2")
expe3 <- c("EmDisc_#1","EmDisc_#2","ActA_MEF","ActA_noMEF","FGF_noMEF","CHIR_MEF","Amnioid_#1","Amnioid_#2","BMP_noMEF","BMP_MEF","SB43_MEF")
...
mat_breaks <- seq(0.65, 0.85, length.out = 20)
pheatmap(C1,color =  redblue1(20),gaps_col=c(2), gaps_row=c(5,8,11,14), border_color = NA, cluster_rows=FALSE, cluster_cols=FALSE,  filename = paste(saveext,"/DimRed/InVivoInvitroCrossRNA",".pdf",sep="") ,width=2,height=5.5)
...
marmoset_data_20307 <- CreateSeuratObject(counts = raw_counts20307[,which(Key20307$QC>0)], assay = "RNA",min.cells = 0, min.features = 0)
Idents(marmoset_data_20307) <- Key20307$Primary.Annotation[which(Key20307$QC>0)]
marmoset_data_20307$Stage <-Key20307$Stage[which(Key20307$QC>0)] #"CS6"
...
marmoset_data_20308 <- CreateSeuratObject(counts = raw_counts20308[,which(Key20308$QC>0)], assay = "RNA",min.cells = 0, min.features = 0)
Idents(marmoset_data_20308) <- Key20308$Primary.Annotation[which(Key20308$QC>0)]
marmoset_data_20308$Stage <- Key20308$Stage[which(Key20308$QC>0)]  #"CS7"
...
marmoset_data_Pre2 <- CreateSeuratObject(counts = raw_countsPre[,which(KeyPre$QC>0)], assay = "RNA",min.cells = 0, min.features = 0)
Idents(marmoset_data_Pre2) <- KeyPre$Primary.lineage[which(KeyPre$QC>0)]
marmoset_data_Pre2$LOC <- KeyPre$Location[which(KeyPre$QC>0)]
marmoset_data_Pre2 <- subset(marmoset_data_Pre2, idents = c("Epi_CS3","cMor_CS3","ICM_CS3"))
marmoset_data_Pre2 <- NormalizeData(marmoset_data_Pre2, verbose = FALSE)
marmoset_data_Pre2$Stage <- "Pre"
marmoset_data_Pre2$Dataset <- "InVivo"
...
marmoset_data_CRUK3$LOC <- KeyCRUK3$Loc[which(KeyCRUK3$QC>0)]
marmoset_data_CRUK3$Stage <- KeyCRUK3$Stage[which(KeyCRUK3$QC>0)]
marmoset_data_CRUK3 <- subset(marmoset_data_CRUK3, idents = c("Am_CS7","EmDisc_CS7","EmDisc_CS7_Gast")) #,"Stroma_CS6")) 
marmoset_data_CRUK3 <- NormalizeData(marmoset_data_CRUK3, verbose = FALSE)
marmoset_data_CRUK3$Dataset <- "InVivo"
...
#saveRDS(marmoset_dataInVivo,"~/Desktop/UpdatedMarmoset.rds")
